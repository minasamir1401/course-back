...
// Helper to safely parse strings into JSON if they look like JSON, otherwise return text or default structure
function parseComplexCell(str, fieldType) {
    if (!str)
        return [];
...
function chunkString(str, size = 32000) {
    if (!str)
        return [''];
...
function formatQuestionsForExcel(qData) {
    let questions = qData;
    ...
    return questions.map((q) => {
        ...
        if (q.difficulty || q.level)
            text += ` | Difficulty: ${q.difficulty || q.level}`;
        if (q.bloomLevel)
            text += ` | Bloom: ${q.bloomLevel}`;
        if (q.explanation)
            text += ` | Explanation: ${q.explanation}`;
        if (q.standard)
            text += ` | Standard: ${q.standard}`;
        if (q.indicator)
            text += ` | Indicator: ${q.indicator}`;
        if (q.learningOutcome)
            text += ` | Outcome: ${q.learningOutcome}`;
        if (q.skill)
            text += ` | Skill: ${q.skill}`;
        if (q.points)
        ...
        if (q.dok)
            text += ` | DOK: ${q.dok}`;
        if (q.videoUrl)
            text += ` | Video: ${q.videoUrl}`;
        return text;
    }).join('\n\n');
}
function formatSlidesForExcel(sData) {
    let slides = sData;
    if (typeof sData === 'string') {
        try {
            slides = JSON.parse(sData);
        }
        catch (_a) {
            return sData;
        }
...
function formatAssignmentsForExcel(aData) {
    let assignments = aData;
    if (typeof aData === 'string') {
        try {
            assignments = JSON.parse(aData);
        }
        catch (_a) {
            return aData;
        }
...
function formatAttachmentsForExcel(aData) {
    let attachments = aData;
    if (typeof aData === 'string') {
        try {
            attachments = JSON.parse(aData);
        }
        catch (_a) {
            return aData;
        }
...
// 1. Download Template
router.get('/api/school/import/template', auth_1.verifyToken, (0, auth_1.checkRole)(['SCHOOL_ADMIN', 'SUPER_ADMIN']), (req, res) => {
    try {
...
router.post('/api/school/import/excel', auth_1.verifyToken, (0, auth_1.checkRole)(['SCHOOL_ADMIN', 'SUPER_ADMIN']), shared_1.multerUpload.single('file'), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        if (!req.file) {
        ...
        for (const row of rows) {
            ...
            (_a = coursesMap.get(courseTitle)) === null || _a === void 0 ? void 0 : _a.push(row);
        }
        const syncedCourseIds = new Set();
        yield prisma.$transaction((tx) => __awaiter(void 0, void 0, void 0, function* () {
            for (const [courseTitle, courseRows] of coursesMap.entries()) {
                const whereClause = { title: courseTitle };
                if (schoolId) {
                    whereClause.schoolId = schoolId;
                }
                let course = yield tx.course.findFirst({ where: whereClause });
                if (!course) {
                    const firstRow = courseRows[0];
                    course = yield tx.course.create({
                        data: {
                            title: courseTitle,
        ...
        for (const cId of syncedCourseIds) {
            (0, db_backup_1.syncCourseToCloud)(cId).catch(() => { });
        }
...
router.get('/api/school/export/course/:id', auth_1.verifyToken, (0, auth_1.checkRole)(['SCHOOL_ADMIN', 'SUPER_ADMIN']), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        ...
        if (course.lessons.length === 0) {
            ...
            for (const lesson of course.lessons) {
                ...
                if (!lessonSlidesRaw || lessonSlidesRaw === '[]' || lessonSlidesRaw === 'null') {
                    const slideBlocks = blocks.filter((b) => b.type === 'TEXT');
                    if (slideBlocks.length > 0) {
                        lessonSlidesRaw = slideBlocks.map((b) => ({
                ...
                if (!lessonQuestionsRaw || lessonQuestionsRaw === '[]' || lessonQuestionsRaw === 'null') {
                    ...
                    if (questionBlocks.length > 0) {
                        lessonQuestionsRaw = questionBlocks.map((b) => {
                            let opts = [];
                            try {
                                opts = JSON.parse(b.options || '[]');
                            }
                            catch (e) { }
                            return {
                                type: b.label || 'MCQ',
                ...
                for (let i = 0; i < maxRows; i++) {
                    dataRows.push([
                        ...
                        i === 0 ? (lesson.videoUrl || '') : '',
                        i === 0 ? (((_a = lesson.order) === null || _a === void 0 ? void 0 : _a.toString()) || '0') : '',
                        ...
                        i === 0 ? (((_b = lesson.duration) === null || _b === void 0 ? void 0 : _b.toString()) || '0') : '',
...
router.get('/api/school/export/lesson/:id', auth_1.verifyToken, (0, auth_1.checkRole)(['SCHOOL_ADMIN', 'SUPER_ADMIN']), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (schoolId && ((_a = lesson.course) === null || _a === void 0 ? void 0 : _a.schoolId) !== schoolId) {
        ...
        for (let i = 0; i < maxRows; i++) {
            dataRows.push([
                ((_b = lesson.course) === null || _b === void 0 ? void 0 : _b.title) || '',
                ((_c = lesson.course) === null || _c === void 0 ? void 0 : _c.grade) || '',
                ((_d = lesson.course) === null || _d === void 0 ? void 0 : _d.subject) || '',
                ((_e = lesson.course) === null || _e === void 0 ? void 0 : _e.description) || '',
                lesson.title || '',
                i === 0 ? (lesson.domain || '') : '',
                i === 0 ? (stripHtml(lesson.summary) || '') : '',
                i === 0 ? (stripHtml(lesson.notes) || '') : '',
                i === 0 ? (stripHtml(lesson.content) || '') : '',
                i === 0 ? (lesson.videoUrl || '') : '',
                i === 0 ? (((_f = lesson.order) === null || _f === void 0 ? void 0 : _f.toString()) || '0') : '',
                i === 0 ? (lesson.publishDate ? lesson.publishDate.toISOString().split('T')[0] : '') : '',
                i === 0 ? (lesson.cutOffDate ? lesson.cutOffDate.toISOString().split('T')[0] : '') : '',
                slidesChunks[i] || '',
                assignChunks[i] || '',
                ...
                i === 0 ? (lesson.indicators || '') : '',
                i === 0 ? (lesson.learningOutcomes || '') : '',
                i === 0 ? (((_g = lesson.duration) === null || _g === void 0 ? void 0 : _g.toString()) || '0') : '',
                i === 0 ? (lesson.isVisible !== false ? 'TRUE' : 'FALSE') : ''
            ]);
        }
        const workbook = xlsx.utils.book_new();
...
router.get('/api/school/export/json/course/:id', auth_1.verifyToken, (0, auth_1.checkRole)(['SCHOOL_ADMIN', 'SUPER_ADMIN', 'TEACHER']), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        const courseId = req.params.id;
        const schoolId = req.user.role === 'SUPER_ADMIN' ? undefined : req.user.schoolId;
        ...
        const exportData = {
            ...
            course: {
                ...
                schoolName: ((_a = course.school) === null || _a === void 0 ? void 0 : _a.name) || null,
                assignedSchools: ((_b = course.schools) === null || _b === void 0 ? void 0 : _b.map((s) => s.name)) || []
            },
...
router.post('/api/school/import/json/course', auth_1.verifyToken, (0, auth_1.checkRole)(['SCHOOL_ADMIN', 'SUPER_ADMIN', 'TEACHER']), shared_1.multerUpload.single('file'), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let jsonData = null;
        if (req.file) {
            const fs = require('fs');
        ...
        const createdCourse = yield prisma.$transaction((tx) => __awaiter(void 0, void 0, void 0, function* () {
            // 1. Check if course already exists by Title and School/Centrality
            let existingCourse = yield tx.course.findFirst({
                where: {
            ...
            for (let idx = 0; idx < lessonsList.length; idx++) {
                ...
                const existingLesson = yield tx.lesson.findFirst({
                    where: {
                        title: l.title,
                        courseId: courseId,
                        deletedAt: null
                ...
                if (existingLesson) {
                    ...
                    yield tx.lesson.create({
                        data: Object.assign(Object.assign({}, lessonData), { title: l.title, courseId: courseId })
                    });
                }
            }
            return yield tx.course.findUnique({ where: { id: courseId } });
        }));
        // Real-time Dual-Write: Instantly sync JSON imported course to Cloud Backup cloud database
        if (createdCourse === null || createdCourse === void 0 ? void 0 : createdCourse.id)
            (0, db_backup_1.syncCourseToCloud)(createdCourse.id).catch(() => { });
