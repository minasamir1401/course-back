        ...
        try {
            ...
            if (schoolCount === 0 && shouldSeedDummyData) {
                ...
                for (const s of schoolsData) {
                    ...
                    yield prisma_1.default.user.create({ data: { username: `${s.subdomain}_admin`, password: yield bcryptjs_1.default.hash(crypto_1.default.randomBytes(10).toString('hex'), 10), name: `أ/ محمد أحمد - مدير ${s.name}`, role: 'SCHOOL_ADMIN', schoolId: school.id } });
                    const teacherNames = ['أحمد محمود', 'سارة حسن', 'إبراهيم علي', 'مريم يوسف'];
                    for (let i = 0; i < teacherNames.length; i++) {
                        yield prisma_1.default.user.create({ data: { username: `${s.subdomain}_teacher_${i + 1}`, password: yield bcryptjs_1.default.hash(crypto_1.default.randomBytes(10).toString('hex'), 10), name: `أ/ ${teacherNames[i]}`, role: 'TEACHER', schoolId: school.id } });
                    }
...
const startServer = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (isPrimaryWorker) {
        initializeStartupData()
            ...
            .catch((error) => console.error('[Startup] Startup data initialization failed:', error.message));
    }
    else {
