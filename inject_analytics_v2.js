...
const routeCode = `
router.get('/api/exams/:id/analytics', verifyToken, checkRole(['SUPER_ADMIN', 'SCHOOL_ADMIN', 'TEACHER']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
if (!content.includes('/api/exams/:id/analytics')) {
