...
const startServer = async () => {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    }, 30_000); // 30 seconds delay â€” safely after healthcheck passes
  });

  // These are fast DB queries â€” run immediately (no deferral needed)
  ensurePerformanceIndexes()
    .then(() => console.log('âœ… Performance indexes are ready'))
    .catch((error: any) => console.error('âš ï¸ Performance index setup failed:', error.message));

  initializeStartupData()
    .then(() => console.log('âœ… Startup data initialized'))
    .catch((error: any) => console.error('âš ï¸ Startup data initialization failed:', error.message));
};
